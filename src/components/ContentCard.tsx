...
export const ContentCard = memo(function ContentCard({
  ...
  return (
    <div
      className={cn(
        'bg-card rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow',
        className
      )}
    >
      <div className={`h-48 ${gradient}`} role="img" aria-label={imageLabel}></div>
      <div className="p-6">
        {category && <span className="text-sm text-primary">{category}</span>}
        <h3 className="text-xl font-bold mt-1">{title}</h3>
        {badge && <span className={`px-2 py-1 ${badgeColor} rounded text-xs`}>{badge}</span>}

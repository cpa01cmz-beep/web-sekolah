...
const MobileCardRow = memo(
  ({
    ...
    return (
      <Card
        key={row.id}
        className={cn('p-4 md:hidden', mobileCardClassName)}
        role="listitem"
        aria-setsize={-1}
      ...
      >
        <CardContent className="p-0 space-y-3">
          {row.cells.map(cell => {
            const headerLabel = headerMap.get(cell.key)
            return (
              <div key={cell.key} className="flex flex-col gap-1">
                ...
                <div className={cn(cell.className)} aria-labelledby={`${row.id}-${cell.key}-label`}>
                  {cell.content}
                </div>
              </div>
            )
  ...
  (prevProps, nextProps) => {
    return (
      prevProps.row.id === nextProps.row.id &&
      prevProps.mobileCardClassName === nextProps.mobileCardClassName &&
      prevProps.headerMap === nextProps.headerMap
    )
  }
)
...
export const ResponsiveTable = memo(function ResponsiveTable({
  ...
  const headerMap = useMemo(() => new Map(headers.map(h => [h.key, h.label])), [headers])
          ...
          <tbody role="rowgroup">
            {rows.map((row, index) => (
              <TableRow
                key={row.id}
                row={row}
                _mobileCardClassName={mobileCardClassName}
                rowIndex={index + 1}
              />
            ))}
          </tbody>

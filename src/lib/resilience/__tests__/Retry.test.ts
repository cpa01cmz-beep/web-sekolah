...
describe('Retry Utility', () => {
  ...
  describe('Timeout Functionality', () => {
    ...
    it('should not timeout when function completes within timeout', async () => {
      const fn = vi.fn().mockImplementation(() => new Promise(resolve => setTimeout(resolve, 50)))
    ...
    it.skip('should handle zero timeout (immediate timeout)', async () => {
      ...
      const fn = vi.fn().mockImplementation(() => new Promise(resolve => setTimeout(resolve, 10)))
  ...
  describe('Integration Scenarios', () => {
    it('should handle rapid successive calls independently', async () => {
      const fn1 = vi.fn().mockRejectedValueOnce(new Error('Error 1')).mockResolvedValue('success 1')

      const fn2 = vi.fn().mockRejectedValueOnce(new Error('Error 2')).mockResolvedValue('success 2')
  ...
  describe('onRetry Callback', () => {
    it('should call onRetry callback before each retry attempt', async () => {
      const fn = vi
        .fn()
        .mockRejectedValueOnce(new Error('Error 1'))
        .mockRejectedValueOnce(new Error('Error 2'))
      ...
      expect(onRetry).toHaveBeenNthCalledWith(1, new Error('Error 1'), 0, 1000)
      expect(onRetry).toHaveBeenNthCalledWith(2, new Error('Error 2'), 1, 2000)
    })

    it('should not call onRetry when no retries occur', async () => {
      const fn = vi.fn().mockResolvedValue('success')
      const onRetry = vi.fn()
    ...
    it('should call onRetry with correct delay including jitter', async () => {
      const fn = vi.fn().mockRejectedValueOnce(new Error('Error')).mockResolvedValue('success')

      const onRetry = vi.fn()
      const promise = withRetry(fn, { maxRetries: 2, baseDelay: 1000, jitterMs: 100, onRetry })
    ...
    it('should work without onRetry callback provided', async () => {
      const fn = vi.fn().mockRejectedValueOnce(new Error('Error')).mockResolvedValue('success')
    ...
    it('should call onRetry even when shouldRetry returns false after first failure', async () => {
      const fn = vi
        .fn()
        .mockRejectedValueOnce(new Error('Error 1'))
        .mockRejectedValueOnce(new Error('Error 2'))
      ...
      const promise = withRetry(fn, { maxRetries: 3, baseDelay: 1000, onRetry, shouldRetry })
      await vi.advanceTimersByTimeAsync(1000)

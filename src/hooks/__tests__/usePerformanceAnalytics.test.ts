...
describe('usePerformanceAnalytics', () => {
  describe('empty data handling', () => {
    ...
    it('should return default values for undefined input', () => {
      const { result } = renderHook(() => usePerformanceAnalytics(undefined as unknown as GradeData[]));
  ...
  describe('memoization', () => {
    ...
    it('should update when grades change', () => {
      const { result, rerender } = renderHook(
        ({ grades }) => usePerformanceAnalytics(grades),
        {
          initialProps: {
      ...
      rerender({
        grades: [{ score: 90, subject: 'Math' }, { score: 90, subject: 'Science' }],
      });

...
## Changes
- Updated `hono` from 4.8.5 to 4.10.5 to fix CVE-2025-59139 (high severity)
- Updated `zod` from 4.0.5 to 4.1.12 for latest security patches
- Auto-updated `jspdf` from 2.5.1 to 3.0.3 via `npm audit fix` (major version change)

## Vulnerabilities Fixed
1. **High Severity**: CVE-2025-59139 in Hono - Request body size limit bypass in bodyLimit middleware
2. **Moderate Severity**: ReDoS vulnerability in Zod (addressed by updating to latest version)
...
- Zod ReDoS vulnerability: Fixed in versions after 3.22.3
...
vi.mock('../integration-monitor', () => ({
  integrationMonitor: {
    recordWebhookEventCreated: vi.fn(),
    updatePendingDeliveries: vi.fn(),
    recordWebhookDelivery: vi.fn(),
    recordWebhookEventProcessed: vi.fn(),
  },
}))

vi.mock('../webhook-crypto', () => {
  return {
    generateSignature: vi.fn().mockImplementation(async (payload: string, secret: string) => {
    ...
    verifySignature: vi
      .fn()
      .mockImplementation(async (payload: string, signature: string, secret: string) => {
        const { generateSignature } = await import('../webhook-crypto')
        try {
...
vi.mock('@shared/CircuitBreaker', () => ({
  CircuitBreaker: {
    createWebhookBreaker: vi.fn(() => ({
      execute: vi.fn(),
    })),
  },
}))
...
=============================================================================
  ...
  describe('triggerEvent method', () => {
    it('should have triggerEvent as a function', async () => {
      const { WebhookService } = await import('../webhook-service')
      expect(typeof WebhookService.triggerEvent).toBe('function')
    })
  })

  describe('processPendingDeliveries method', () => {
    it('should have processPendingDeliveries as a function', async () => {
      const { WebhookService } = await import('../webhook-service')
      expect(typeof WebhookService.processPendingDeliveries).toBe('function')
    })
  })
  ...
  describe('generateSignature function', () => {
    it('should generate HMAC signature', async () => {
      const payload = JSON.stringify({ id: 'event-001', type: 'test' })
    ...
    it('should generate different signatures for different payloads', async () => {
      ...
      const sig1 = await generateSignature(JSON.stringify({ id: '1' }), secret)
      const sig2 = await generateSignature(JSON.stringify({ id: '2' }), secret)
    ...
    it('should generate different signatures for different secrets', async () => {
      ...
      const sig1 = await generateSignature(payload, 'secret-1')
      const sig2 = await generateSignature(payload, 'secret-2')
  ...
  describe('verifySignature function', () => {
    ...
    it('should reject invalid signature', async () => {
      const isValid = await verifySignature('{"id":"event"}', 'sha256=invalid', 'secret')
    ...
    it('should reject tampered payload', async () => {
      ...
      const isValid = await verifySignature(JSON.stringify({ id: 'event-002' }), signature, secret)

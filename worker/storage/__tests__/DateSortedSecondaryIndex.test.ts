...
describe('DateSortedSecondaryIndex', () => {
  ...
  describe('count', () => {
    ...
    it('should return correct count for single entry', async () => {
      mockStub.listPrefix.mockResolvedValue({
        keys: ['sort:90000000000000000001:entity-1'],
        next: null,
      })
  ...
  describe('getAll', () => {
    it('should return all entity IDs sorted by date (newest first)', async () => {
      ...
      mockStub.listPrefix.mockResolvedValue({ keys: mockKeys, next: null })

      const result = await index.getAll()

      expect(mockStub.listPrefix).toHaveBeenCalledWith('sort:')
      expect(result).toEqual(['entity-2', 'entity-1', 'entity-3'])
    })

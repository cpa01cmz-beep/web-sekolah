...
export function authRoutes(app: Hono<{ Bindings: Env }>) {
  app.get(
    '/api/auth/verify',
    withErrorHandler('verify auth token')(async (c: Context) => {
  ...
  app.post(
    '/api/auth/login',
    withErrorHandler('login user')(async (c: Context) => {
      ...
      const isPasswordValid = await verifyPasswordTimingSafe(password, user.passwordHash)

...
export class GradeEntity extends IndexedEntity<Grade> {
  ...
  static readonly initialState: Grade = {
    id: '',
    studentId: '',
    courseId: '',
    score: 0,
  ...
  static async existsByStudentIdAndCourseId(
    env: Env,
    studentId: string,
    courseId: string
  ): Promise<boolean> {
    const index = new CompoundSecondaryIndex(env, this.entityName, ['studentId', 'courseId'])
    return await index.existsByValues([studentId, courseId])
  }

  static async countByStudentIdAndCourseId(
    env: Env,
    studentId: string,
    courseId: string
  ): Promise<number> {
    const index = new CompoundSecondaryIndex(env, this.entityName, ['studentId', 'courseId'])
    return await index.countByValues([studentId, courseId])
  }

  static async getByStudentIdAndCourseId(
    env: Env,
    studentId: string,
    courseId: string
  ): Promise<Grade | null> {

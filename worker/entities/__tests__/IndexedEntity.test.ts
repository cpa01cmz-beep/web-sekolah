...
describe('IndexedEntity', () => {
  ...
  describe('existsBySecondaryIndex', () => {
    ...
    it('should return false when no entities exist with the given field value', async () => {
      ...
      const result = await TestIndexedEntity.existsBySecondaryIndex(
        mockEnv,
        'category',
        'nonexistent'
      )
  ...
  describe('countBySecondaryIndex', () => {
    it('should return the count of entities with the given field value', async () => {
      mockStub.listPrefix.mockResolvedValue({
        keys: ['field:cat-a:entity:id-1', 'field:cat-a:entity:id-2'],
      })
    ...
    it('should return 0 when no entities exist with the given field value', async () => {
      ...
      const result = await TestIndexedEntity.countBySecondaryIndex(
        mockEnv,
        'category',
        'nonexistent'
      )
  ...
  describe('update', () => {
    it('should update entity and not modify secondary index when indexed field unchanged', async () => {
      const initialState = {
        id: 'id-1',
        name: 'Test 1',
        category: 'cat-a',
      ...
      mockStub.getDoc
        .mockResolvedValueOnce({ v: 1, data: initialState })
        .mockResolvedValueOnce({ v: 1, data: initialState })
        .mockResolvedValueOnce({ v: 2, data: updatedState })
      mockStub.casPut.mockResolvedValue({ ok: true, v: 2 })
      mockStub.listPrefix.mockResolvedValue({ keys: [] })

      const result = await TestIndexedEntity.update(mockEnv, 'id-1', { name: 'Updated Name' })
    ...
    it('should update secondary index when indexed field changes', async () => {
      const initialState = {
        id: 'id-1',
        name: 'Test 1',
        category: 'cat-a',
      ...
      const result = await TestIndexedEntity.update(mockEnv, 'id-1', { category: 'cat-b' })

      expect(result).not.toBeNull()
      expect(result?.category).toBe('cat-b')
      expect(mockStub.del).toHaveBeenCalledWith('field:cat-a:entity:id-1')
      expect(mockStub.casPut).toHaveBeenCalledWith('field:cat-b:entity:id-1', 0, {
        entityId: 'id-1',
      })
    })

    it('should not update secondary index when indexed field value is same', async () => {
      const initialState = {
        id: 'id-1',
        name: 'Test 1',
        category: 'cat-a',
      ...
      const result = await TestIndexedEntity.update(mockEnv, 'id-1', { category: 'cat-a' })
    ...
    it('should return null when trying to update soft-deleted entity', async () => {
      mockStub.getDoc.mockResolvedValue({
        v: 1,
        data: {
          id: 'id-1',
      ...
      const result = await TestIndexedEntity.update(mockEnv, 'id-1', { name: 'Updated' })
  ...
  describe('getByIds', () => {
    ...
    it('should return map of entities for valid ids', async () => {
      ...
      expect(result.get('id-2')).toEqual({
        id: 'id-2',
        name: 'Test 2',
        category: 'cat-b',
        createdAt: '',
    ...
    it('should handle errors gracefully and skip failed fetches', async () => {
      ...
      mockStub.has.mockResolvedValueOnce(true).mockImplementationOnce(() => {
        throw new Error('Storage error')
      })
    ...
    it('should handle non-existent entities (has returns false)', async () => {
      ...
      mockStub.has.mockResolvedValueOnce(true).mockResolvedValueOnce(false)
      mockStub.getDoc.mockResolvedValueOnce({
        v: 1,
        data: {
          id: 'id-1',

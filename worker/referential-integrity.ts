...
export class ReferentialIntegrity {
  ...
  static async validateGrade(
    ...
    const [student, course] = await Promise.all([
      new UserEntity(env, grade.studentId).getState(),
      new CourseEntity(env, grade.courseId).getState(),
    ])
  ...
  static async checkDependents(
    ...
    if (entity === 'user') {
      ...
      if (user.role === 'teacher') {
        const [classes, courses, announcements] = await Promise.all([
          ClassEntity.getByTeacherId(env, id),
          CourseEntity.getByTeacherId(env, id),
          AnnouncementEntity.getByAuthorId(env, id),
        ])
        if (classes.length > 0) {

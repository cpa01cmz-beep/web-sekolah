  ...
  static async checkDependents(
    ...
    if (entity === 'user') {
      ...
      if (user.role === 'student') {
        ...
        const parents = await UserEntity.getByRole(env, 'parent')
        const linkedParents = parents.filter(p => 'childId' in p && p.childId === id)
        if (linkedParents.length > 0) {
          warnings.push(`This student is linked to ${linkedParents.length} parent account(s)`)
        }
      }
      ...
      if (user.role === 'parent') {
        const parent = user as typeof user & { childId?: string }
        if (parent.childId) {
          const child = await new UserEntity(env, parent.childId).getState()
          if (child) {
            warnings.push(`This parent is linked to child: ${child.name}`)
          } else {
            warnings.push(`This parent is linked to a child that no longer exists`)
          }
        }
      }
    }

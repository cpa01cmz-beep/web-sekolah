...
export function adminMonitoringRoutes(app: Hono<{ Bindings: Env }>) {
  ...
  adminRouter.get('/health', withErrorHandler('retrieve health metrics')(async (c: Context) => {
  ...
  adminRouter.get('/circuit-breaker', withErrorHandler('retrieve circuit breaker state')(async (c: Context) => {
    const metrics = integrationMonitor.getHealthMetrics();
  ...
  adminRouter.post('/circuit-breaker/reset', withErrorHandler('request circuit breaker reset')(async (c: Context) => {
    return ok(c, {
  ...
  adminRouter.get('/rate-limit', withErrorHandler('retrieve rate limit stats')(async (c: Context) => {
    const metrics = integrationMonitor.getHealthMetrics();
  ...
  adminRouter.get('/webhooks', withErrorHandler('retrieve webhook stats')(async (c: Context) => {
    const metrics = integrationMonitor.getHealthMetrics();
  ...
  adminRouter.get('/errors', withErrorHandler('retrieve error stats')(async (c: Context) => {
    const metrics = integrationMonitor.getHealthMetrics();
  ...
  adminRouter.get('/summary', withErrorHandler('retrieve integration summary')(async (c: Context) => {
    const metrics = integrationMonitor.getHealthMetrics();
  ...
  adminRouter.post('/reset-monitor', withErrorHandler('reset integration monitor')(async (c: Context) => {

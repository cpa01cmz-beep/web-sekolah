...
export function teacherRoutes(app: Hono<{ Bindings: Env }>) {
  ...
  app.get('/api/classes/:id/students', ...withUserValidation('teacher', 'class-students'), withErrorHandler('get class students')(async (c: Context) => {
    const classId = c.req.param('id');
    const teacherId = c.req.query('teacherId');
    
    if (!teacherId) {
      return bad(c, 'teacherId is required');
    }

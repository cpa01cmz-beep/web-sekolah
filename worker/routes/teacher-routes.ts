...
import type { TeacherDashboardData, Announcement, CreateAnnouncementData } from "@shared/types";
...
import { createAnnouncementSchema, paramsSchema } from '../middleware/schemas';
...
export function teacherRoutes(app: Hono<{ Bindings: Env }>) {
  ...
  app.get('/api/classes/:id/students', ...withAuth('teacher'), validateParams(paramsSchema), withErrorHandler('get class students')(async (c: Context) => {

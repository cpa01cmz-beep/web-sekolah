    ...
    withErrorHandler('get student card')(async (c: Context) => {
      ...
      const cardData: StudentCardData = {
        ...
        gradeDistribution: {
          ...
          B: grades.filter((g) => g.score >= GRADE_B_THRESHOLD && g.score < GRADE_A_THRESHOLD)
            .length,
          C: grades.filter((g) => g.score >= GRADE_C_THRESHOLD && g.score < GRADE_B_THRESHOLD)
            .length,
          D: grades.filter((g) => g.score >= PASSING_SCORE_THRESHOLD && g.score < GRADE_C_THRESHOLD)
            .length,

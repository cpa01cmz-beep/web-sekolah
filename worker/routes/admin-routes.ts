...
export function adminRoutes(app: Hono<{ Bindings: Env }>) {
  app.post(
    ...
    ...withAuth('admin'),
    withErrorHandler('rebuild indexes')(async (c: Context) => {
  ...
  app.post(
    '/api/admin/rebuild-indexes/:entity',
    ...withAuth('admin'),
    withErrorHandler('rebuild entity indexes')(async (c: Context) => {
      const entityName = c.req.param('entity') as EntityName
      const success = await rebuildEntityIndexes(c.env, entityName)
      if (!success) {
        return bad(
          c,
          `Unknown entity: ${entityName}. Supported entities: ${getSupportedEntityNames().join(', ')}`
        )
      }
  ...
  app.get(
    '/api/admin/rebuild-indexes/entities',
    ...withAuth('admin'),
    withErrorHandler('get supported entities')(async (c: Context) => {
  ...
  app.get(
    '/api/admin/dashboard',
    ...withAuth('admin'),
    withErrorHandler('get admin dashboard')(async (c: Context) => {
      const [totalStudents, totalTeachers, totalParents, totalAdmins, recentAnnouncements] =
        await Promise.all([
  ...
  app.get(
    ...
    withErrorHandler('get admin users')(async (c: Context) => {
      const { role, classId, search } = c.get('validatedQuery') as {
        role?: UserRole
        classId?: string
        search?: string
      }
  ...
  app.get(
    ...
    ...withAuth('admin'),
    withErrorHandler('get admin announcements')(async (c: Context) => {
  ...
  app.get(
    '/api/admin/settings',
    ...withAuth('admin'),
    withErrorHandler('get admin settings')(async (c: Context) => {
  ...
  app.put(
    ...
    withErrorHandler('update admin settings')(async (c: Context) => {

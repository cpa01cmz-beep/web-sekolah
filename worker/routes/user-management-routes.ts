...
export function userRoutes(app: Hono<{ Bindings: Env }>) {
  app.get('/api/users', authenticate(), authorize('admin'), async (c: Context) => {
    const users = await UserService.getAllUsers(c.env);
    return ok(c, users);
  });

  app.post('/api/users', authenticate(), authorize('admin'), async (c: Context) => {
  ...
  app.put('/api/users/:id', authenticate(), authorize('admin'), async (c: Context) => {
  ...
  app.delete('/api/users/:id', authenticate(), authorize('admin'), async (c: Context) => {
    const userId = c.req.param('id');
  ...
  app.put('/api/grades/:id', authenticate(), authorize('teacher'), async (c: Context) => {
  ...
  app.post('/api/grades', authenticate(), authorize('teacher'), async (c: Context) => {

...
async function rebuildUserIndexes(env: Env): Promise<void> {
  ...
  const parentIdIndex = new SecondaryIndex<string>(env, UserEntity.entityName, 'parentId')

  await roleIndex.clear()
  await classIdIndex.clear()
  await emailIndex.clear()
  await parentIdIndex.clear()
  ...
  const parentIdItems = validUsers
    .filter(u => isStudent(u) && u.parentId)
    .map(u => ({ fieldValue: u.parentId!, entityId: u.id }))
  if (parentIdItems.length > 0) {
    await parentIdIndex.addBatch(parentIdItems)
  }
}
...
async function rebuildMessageIndexes(env: Env): Promise<void> {
  ...
  await recipientIsReadCompoundIndex.addBatch(
    validMessages.map(m => ({ fieldValues: [m.recipientId, m.isRead.toString()], entityId: m.id }))
  )

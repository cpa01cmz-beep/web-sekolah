...
async function rebuildClassIndexes(env: Env): Promise<void> {
  ...
  await teacherIdIndex.addBatch(
    validClasses.map((c) => ({ fieldValue: c.teacherId, entityId: c.id }))
  );
}
...
async function rebuildCourseIndexes(env: Env): Promise<void> {
  ...
  await teacherIdIndex.addBatch(
    validCourses.map((c) => ({ fieldValue: c.teacherId, entityId: c.id }))
  );
}
...
async function rebuildGradeIndexes(env: Env): Promise<void> {
  ...
  const compoundIndex = new CompoundSecondaryIndex(env, GradeEntity.entityName, [
    'studentId',
    'courseId',
  ]);
  ...
  await studentIdIndex.addBatch(
    validGrades.map((g) => ({ fieldValue: g.studentId, entityId: g.id }))
  );
  await courseIdIndex.addBatch(
    validGrades.map((g) => ({ fieldValue: g.courseId, entityId: g.id }))
  );
  await compoundIndex.addBatch(
    validGrades.map((g) => ({ fieldValues: [g.studentId, g.courseId], entityId: g.id }))
  );
  ...
  await Promise.all(
    Array.from(gradesByStudent.entries()).map(async ([studentId, studentGrades]) => {
      ...
      await studentDateIndex.addBatch(
        studentGrades.map((g) => ({ date: g.createdAt, entityId: g.id }))
      );
    })
  );
}
...
async function rebuildAnnouncementIndexes(env: Env): Promise<void> {
  ...
  const targetRoleIndex = new SecondaryIndex<string>(
    env,
    AnnouncementEntity.entityName,
    'targetRole'
  );
  ...
  await authorIdIndex.addBatch(
    validAnnouncements.map((a) => ({ fieldValue: a.authorId, entityId: a.id }))
  );
  await targetRoleIndex.addBatch(
    validAnnouncements.map((a) => ({ fieldValue: a.targetRole, entityId: a.id }))
  );
...
async function rebuildWebhookConfigIndexes(env: Env): Promise<void> {
  ...
  await activeIndex.addBatch(
    validConfigs.map((c) => ({ fieldValue: c.active.toString(), entityId: c.id }))
  );
}

async function rebuildWebhookEventIndexes(env: Env): Promise<void> {
  const processedIndex = new SecondaryIndex<string>(
    env,
    WebhookEventEntity.entityName,
    'processed'
  );
  const eventTypeIndex = new SecondaryIndex<string>(
    env,
    WebhookEventEntity.entityName,
    'eventType'
  );
  ...
  await processedIndex.addBatch(
    validEvents.map((e) => ({ fieldValue: e.processed.toString(), entityId: e.id }))
  );
  await eventTypeIndex.addBatch(
    validEvents.map((e) => ({ fieldValue: e.eventType, entityId: e.id }))
  );
}
...
async function rebuildWebhookDeliveryIndexes(env: Env): Promise<void> {
  ...
  const webhookConfigIdIndex = new SecondaryIndex<string>(
    env,
    WebhookDeliveryEntity.entityName,
    'webhookConfigId'
  );
  const idempotencyKeyIndex = new SecondaryIndex<string>(
    env,
    WebhookDeliveryEntity.entityName,
    'idempotencyKey'
  );
  ...
  await statusIndex.addBatch(
    validDeliveries.map((d) => ({ fieldValue: d.status, entityId: d.id }))
  );
  await eventIdIndex.addBatch(
    validDeliveries.map((d) => ({ fieldValue: d.eventId, entityId: d.id }))
  );
  await webhookConfigIdIndex.addBatch(
    validDeliveries.map((d) => ({ fieldValue: d.webhookConfigId, entityId: d.id }))
  );
...
async function rebuildDeadLetterQueueIndexes(env: Env): Promise<void> {
  const webhookConfigIdIndex = new SecondaryIndex<string>(
    env,
    DeadLetterQueueWebhookEntity.entityName,
    'webhookConfigId'
  );
  const eventTypeIndex = new SecondaryIndex<string>(
    env,
  ...
  await webhookConfigIdIndex.addBatch(
    validItems.map((i) => ({ fieldValue: i.webhookConfigId, entityId: i.id }))
  );
  await eventTypeIndex.addBatch(
    validItems.map((i) => ({ fieldValue: i.eventType, entityId: i.id }))
  );
}
...
async function rebuildMessageIndexes(env: Env): Promise<void> {
  ...
  const parentMessageIdIndex = new SecondaryIndex<string>(
    env,
    MessageEntity.entityName,
    'parentMessageId'
  );
  const recipientIsReadCompoundIndex = new CompoundSecondaryIndex(env, MessageEntity.entityName, [
    'recipientId',
    'isRead',
  ]);
  ...
  await senderIdIndex.addBatch(
    validMessages.map((m) => ({ fieldValue: m.senderId, entityId: m.id }))
  );
  await recipientIdIndex.addBatch(
    validMessages.map((m) => ({ fieldValue: m.recipientId, entityId: m.id }))
  );
  ...
  await recipientIsReadCompoundIndex.addBatch(
    validMessages.map((m) => ({
      fieldValues: [m.recipientId, m.isRead.toString()],
      entityId: m.id,
    }))
  );
  ...
  await Promise.all([
    ...Array.from(messagesBySender.entries()).map(async ([senderId, senderMessages]) => {
      const sentDateIndex = new UserDateSortedIndex(
        env,
        MessageEntity.entityName,
        senderId,
        'sent'
      ...
      await sentDateIndex.addBatch(
        senderMessages.map((m) => ({ date: m.createdAt, entityId: m.id }))
      );
    }),
    ...Array.from(messagesByRecipient.entries()).map(async ([recipientId, recipientMessages]) => {
      ...
      await receivedDateIndex.addBatch(
        recipientMessages.map((m) => ({ date: m.createdAt, entityId: m.id }))
      );
    }),
  ]);
}
